...
func (r *VoteRepository) Create(userID int64, question1ID, question2ID, tournamentID int, selectedID *int) error {
	query := `
		INSERT INTO votes (user_id, question1_id, question2_id, tournament_id, selected_id, created_at)
		VALUES (?, ?, ?, ?, ?, ?)
	`
...
func (r *VoteRepository) GetRecentVotes(userID int64, tournamentID int, limit int) ([]*Vote, error) {
	query := `
		SELECT id, user_id, question1_id, question2_id, tournament_id, selected_id, created_at
		FROM votes 
		WHERE user_id = ? AND tournament_id = ?
